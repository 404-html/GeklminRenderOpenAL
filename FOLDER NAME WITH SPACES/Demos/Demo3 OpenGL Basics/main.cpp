...
void init(){
	...
	myDevice->addWindow(WIDTH, HEIGHT, "Hello World Window");//Create the window!
...
int main(){
	...
	//CPU-side Triangle Data
		GLfloat TrianglePositionData[12] = {
			-0.90f, -0.90f,0.5, 1, //NOTE: Making W 0 is BAD JUJU to opengl for some reason
			0.90f, -0.90f,0.5, 1,
			-0.0f,  0.90f,0.5, 1
		};
	...
	//I'm using the GeklminRender Shader class for convenience. You can use your own OpenGL Code to load the shader and compile it, but I prefer not to.
		Shader* TriangleShader = new Shader("shaders/triangles");
		TriangleShader->Bind();//Bind it to work with it...
		
	//Generate the Triangle's VAO- Typically (including in GeklminRender's Mesh Class) a Vertex Array Object represents one "Object". VBOs are typically tied to the one VAO as individual or multiple Vertex Attributes.
		glGenVertexArrays( 
			1, //How many to generate
			&TriangleVAO //Where to write the handle 
		); // Go pick a spot in memory, give it a name, and put it in the VAOs array. Do that NumVAOs times.
	...
	//Let's set up VBO 1: Positions
		...
		glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, 0, 0); //Tells the VAO how to get its 0'th vertex attribute out of this VBO.
	//Let's set up VBO 2: Colors
		glBindBuffer( GL_ARRAY_BUFFER, TriangleVBOs[1] );
		glBufferData( GL_ARRAY_BUFFER, sizeof(TriangleColorData), TriangleColorData, GL_STATIC_DRAW);//Data Upload
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, 0);//Tells the VAO how to get its 1st vertex attribute out of this VBO.
	...
	//Always ALWAYS unbind the vertex array
		glBindVertexArray(0);
	//We don't actually NEED these but for 99.9% of cases you are going to use them
