...
struct Sphere : public TraceableShape{
	...
	float radius = 1;
	glm::vec3 FindClosestCollisionPoint(glm::vec3 RayDir, glm::vec3 EyePos, float drawdistance) { //RAYDIR MUST BE NORMALIZED!!!
		float distance = 0;
	...
	glm::vec3 GetNormalAtPoint(glm::vec3 PointOnSurface){
		return glm::normalize(PointOnSurface - position);
	}
	...
	glm::vec3 GetColorAtPoint(glm::vec3 PointOnSurface){
		return color;
	}
	// float raySphereIntersect(glm::vec3 r0, glm::vec3 rd, glm::vec3 s0, float sr) { //found on internet
		// - r0: ray origin
...
struct TracerWorld{
	...
	float TraceRayDistance2(glm::vec3 RayDir, glm::vec3 EyePos, float renderdistance) //Finds the closest object and gets the distance... used for shadows only
	{
		//bool didFind = false;
		glm::vec3 closestCollisionPoint = EyePos;
		//size_t indexofclosestobject = TracerObjects.size(); //This value will fail the test
		float closestDistance = -1;
		//Test against all the TracerObjects to find the closest collision point to the EyePos
		...
		// if (!didFind)
			// std::cout << "\n DIDNT FIND ANYTHING";
		//Now that we have the closest collision point in the scene, return the distance
		return closestDistance;
	}
	
	
	glm::vec3 TraceRay(glm::vec3 RayDir, glm::vec3 EyePos, int RayCountDown = 5, float renderdistance = 300) //Bounces 5 times, returns a color. Finds the closest collision point
	{
		// if(RayCountDown < 3)
					// std::cout << "\nRAYDIR:" <<
					// "\nX: " << RayDir.x << 
		...
		//Now that we have the closest collision point in the scene, use it to compute the ray color. Bounce, of course...
		if(RayCountDown > 0 && didFind && indexofclosestobject < TracerObjects.size())
		{
			...
			for (size_t i = 0; i < TracerLights.size(); i++)
			{
				...
				if ((disttoclosestobject > disttolight || disttoclosestobject < 0) && dotproduct > 0)
				{
					...
					*/
					...
					//std::cout << "\ndotproduct:" << glm::dot(reflectedLightDir, -RayDir);
			...
			glm::vec3 returnval = (LightingMultiplier * TracerObjects[indexofclosestobject]->GetColorAtPoint(closestCollisionPoint)) * (1-TracerObjects[indexofclosestobject]->reflectivity) + TracerObjects[indexofclosestobject]->reflectivity * ( LightingAdder +
				TraceRay(
					BouncedRay,
			...
			return glm::clamp(TracerObjects[indexofclosestobject]->GetColorAtPoint(closestCollisionPoint) * LightingMultiplier + TracerObjects[indexofclosestobject]->reflectivity * LightingAdder, glm::vec3(0), glm::vec3(1));
		}
