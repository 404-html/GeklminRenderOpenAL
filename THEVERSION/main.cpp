...
void LoadResources()
{
	...
	*/
	...
	WaterSurfaceInstance.myPhong.specreflectivity = -0.9; //NEGATIVE MEANS OBJECTS ARE SHINIER AND LESS TRANSPARENT AT SHALLOW ANGLES
	WaterSurfaceInstance.myPhong.emissivity = 5.0; //Power to use for ShallowShiny effect
		...
		//GAME CODE | Demonstrates using transforms to set the rotation and position of an object in the scene, as well as properties and changing shape.
		{
				...
				for(int i = 0; i < FLUID_WIDTH; i++)
					for(int j = 0; j < FLUID_HEIGHT; j++)
					{
						...
						Fluid_Target[(i + j * FLUID_WIDTH) * 4 + 3] = 100;
