...
namespace GeklminRender {
	...
	FBO::FBO(int width, int height, unsigned int howmanycolorattachments, GLenum InternalDataType)
	{
		...
		{
			numcolorattachments = howmanycolorattachments; //How many color attachments will we allocate?
		}
		...
		for (unsigned int i = 0; i<numcolorattachments; i++){
			...
			glTexImage2D(GL_TEXTURE_2D, 0, InternalDataType, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0); //Create the GPU memory space allocation
	...
	FBO::FBO(int width, int height, unsigned int howmanycolorattachments, GLenum InternalDataType, GLuint mylittledepthbuffer) {
		...
		glDrawBuffers(numcolorattachments, DrawBuffers); // "1" is the size of DrawBuffers
	...
	void FBO::BindDrawBuffers(){
			glDrawBuffers(numcolorattachments, DrawBuffers); //it broke things, and it ain't pretty;
		}
	void FBO::BindRenderTarget(){
			...
			glBindFramebuffer(GL_FRAMEBUFFER, m_FBO);
			glDrawBuffers(numcolorattachments, DrawBuffers); //it broke things, and it ain't pretty
			glViewport(0,0,width,height); //The thing about screenquads probably isn't true, and even if it is, we can adjust for it.
		}
	...
	GLuint FBO::getHandle(unsigned int whichone)//Used for deregistering the safetextures from the meshes. Hotfix for 106 release
	{
		return m_RenderTex[whichone];
	}
};