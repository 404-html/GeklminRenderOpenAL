...
void main()
{
	...
	for (int i = 0; i < MAX_DIR_LIGHTS && i < numdirlights; i++)
	{
		...
		float inthelist = float(
								// (length(dir_lightArray[i].sphere1.xyz - worldout) < dir_lightArray[i].sphere1.w) || 
								// (length(dir_lightArray[i].sphere2.xyz - worldout) < dir_lightArray[i].sphere2.w) || 
								// (length(dir_lightArray[i].sphere3.xyz - worldout) < dir_lightArray[i].sphere3.w) || 
								// (length(dir_lightArray[i].sphere4.xyz - worldout) < dir_lightArray[i].sphere4.w) ||
								( //AABB 1&2
	...
	for (int i = 0; i < MAX_POINT_LIGHTS && i < numpointlights; i++)
	{
		...
		float inthelist = float(
								// (length(point_lightArray[i].sphere1.xyz - worldout) < point_lightArray[i].sphere1.w) || 
								// (length(point_lightArray[i].sphere2.xyz - worldout) < point_lightArray[i].sphere2.w) || 
								// (length(point_lightArray[i].sphere3.xyz - worldout) < point_lightArray[i].sphere3.w) || 
								// (length(point_lightArray[i].sphere4.xyz - worldout) < point_lightArray[i].sphere4.w) ||
								// ( //AABB 1&2
		...
		float rangevar = pow(max(1-(lightdist/point_lightArray[i].range),0.0), point_lightArray[i].dropoff);
	...
	for (int i = 0; i < MAX_AMB_LIGHTS && i < numamblights; i++){
		float inthelist = float(
								// (length(amb_lightArray[i].sphere1.xyz - worldout) < amb_lightArray[i].sphere1.w) || 
								// (length(amb_lightArray[i].sphere2.xyz - worldout) < amb_lightArray[i].sphere2.w) || 
								// (length(amb_lightArray[i].sphere3.xyz - worldout) < amb_lightArray[i].sphere3.w) || 
								// (length(amb_lightArray[i].sphere4.xyz - worldout) < amb_lightArray[i].sphere4.w) ||
								// ( //AABB 1&2
		...
		float inrange = float(length(fragtolight) < amb_lightArray[i].range);
	...
	vec3 specularcolorval = speceffect + cubemapData.xyz * enableCubeMapReflections * specreflectivity * specdamp/128.0;
	specularcolorval = min(specularcolorval, vec3(1));
	vec3 NewfragColor = primary_color + specularcolorval;
	NewfragColor = clamp(NewfragColor, vec3(0), vec3(1));
		...
		float weight = 
			  ...
			  clamp(0.03 / (1e-5 + pow(gl_FragDepth / 200, 4.0)), 1e-2, 3e3);
	...
	//Outdated comment, ignore
			//If you want multiplicative transparency, highlight the two lines in SceneAPI.cpp with the glBlendFunc functions, and then hit control+Q (Comments uncommented lines and uncomments commented lines). Change the vec3(0) to vec3(1).
